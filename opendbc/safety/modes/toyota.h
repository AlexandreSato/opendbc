...
static bool toyota_tx_hook(const CANPacket_t *msg) {
  ...
  if (addr == 0x750) {
    ...
    bool valid_uds_msgs = (GET_BYTES(msg, 0, 4) == 0x11300540U); // un/lock doors
    valid_uds_msgs |= (GET_BYTES(msg, 0, 4) == 0x06300440U); // horn
    valid_uds_msgs |= (GET_BYTES(msg, 0, 4) == 0x15300640U); // highbeam
